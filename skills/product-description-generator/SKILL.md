...
For Thai Shopee or Lazada listing work, the following checks are mandatory:

- Establish the current product truth table first. Separate confirmed facts, unknowns and prohibited inferences. The newest explicit user confirmation overrides an older project brief; a product-specific restriction must never become a universal restriction.
- Use the platform output contract: one title no longer than 120 Unicode characters, five highlights, a complete description, a specification block, color/variant information and purchase notes. If a requested section is missing, do not call the listing complete.
- Put the primary Thai product keyword first, use natural Thai marketplace phrasing, include confirmed measurable parameters, and avoid keyword stuffing. Do not invent a brand, model, certification, exact usable capacity or performance level.
- Distinguish outer dimensions from usable capacity. Dimensions may support a size description or a clearly qualified geometric estimate, but they do not automatically prove an exact internal litre capacity.
- If live keyword research is used, report the observed keyword patterns separately from the product evidence and exclude competitor brands. If research is unavailable, do not fabricate search volume or ranking claims.
- Validate the final title character count, Thai spelling/spacing, numerals, units, claim scope and keyword coverage. Provide a Chinese review translation separately; never mix Chinese into the customer-facing Thai copy.
